...
create table customer
 (cust_id serial  PRIMARY KEY,
  ...
  constraint check_allowed check ( cust_type_cd in ('I','B'))
 );
...
create table account
 (account_id serial  PRIMARY KEY,
  ...
  constraint check_allowed check ( status in ('ACTIVE','CLOSED','FROZEN'))
 );

/* Direct Benefit Transfer (DBT)
Certified of Deposit (CDT)*/
create table transaction
 (txn_id serial  PRIMARY KEY,
  txn_date timestamp not null,
  account_id integer  not null,
  txn_type_cd varchar(4) not null,
  ...
  constraint fk_exec_branch_id foreign key (execution_branch_id)
    references branch (branch_id),
  constraint check_allowed check ( txn_type_cd in ('DBT','CDT') )
 );
...
--create temporary table emp_tmp_table as
--select emp_id, fname, lname from employee;
/*alternative*/

with emp_tmp AS ( select emp_id, fname, lname from employee)
...
select a.prod_cd, c.cust_id, cast(a.open_date as timestamp), cast( a.last_date as timestamp), 'ACTIVE',
  e.branch_id, e.emp_id, a.avail, a.pend
from customer c , 
 (select b.branch_id, e.emp_id 
  from branch b , employee e where e.assigned_branch_id = b.branch_id
  and b.city = 'Woburn' limit 1) e
  ,
